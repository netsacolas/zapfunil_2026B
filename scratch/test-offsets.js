...
async function testOffset(limit, offset) {
  try {
    console.log(`Fetching chats/overview with limit=${limit}, offset=${offset}...`);
    const startTime = Date.now();
    const res = await fetch(`${wahaUrl}/api/${sessionName}/chats/overview?limit=${limit}&offset=${offset}`, {
      headers: {
        'x-api-key': apiKey,
...
async function run() {
  await testOffset(15, 0);
  await testOffset(15, 15);
}
