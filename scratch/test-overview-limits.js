...
async function testLimit(limit) {
  try {
    console.log(`Fetching chats/overview with limit=${limit}...`);
    const startTime = Date.now();
    const res = await fetch(`${wahaUrl}/api/${sessionName}/chats/overview?limit=${limit}`, {
      headers: {
        'x-api-key': apiKey,
    ...
    console.log(`Limit: ${limit} | Status: ${res.status} | Time: ${duration}s`);
    if (res.ok) {
    ...
    console.error(`Error for limit ${limit}:`, err.message);
  }
}
